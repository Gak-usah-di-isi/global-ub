...
@section('content')
    <div class="space-y-6">
        <nav class="text-[12px] font-medium flex items-center gap-1 text-slate-500">
            ...
            <span class="text-slate-700">Create Testimonial</span>
        </nav>
        ...
        <form id="create-form" action="{{ route('testimonials.store') }}" method="POST" enctype="multipart/form-data"
            class="space-y-8">
            @csrf
            <section class="bg-white shadow-card rounded-xl p-6 md:p-8">
                ...
                <div class="space-y-7">
                    ...
                    <div>
                        <label for="position" class="field-label req">
                            Position
                        </label>
                    ...
                    <div>
                        <label for="organization" class="field-label req">
                            Organization
                        </label>
                    ...
                    <div>
                        <label for="testimonial_type" class="field-label req">
                            Testimonial Type
                        </label>
                        <input id="testimonial_type" name="testimonial_type" type="text"
                            value="{{ old('testimonial_type') }}" placeholder="Enter testimonial type"
                    ...
                    <div>
                        <label for="testimonial" class="field-label req">
                            Testimonial
                        </label>
                        <textarea id="testimonial" name="testimonial" rows="4" placeholder="Enter testimonial content"
                            class="mt-2 w-full rounded-lg border border-[#E3E9F2] bg-[#F8FAFE] px-4 py-2 text-[13px] text-slate-800 placeholder:text-slate-400 focus:border-primary-500 focus:bg-white focus:ring-primary-500/15 outline-none transition-colors resize-none @error('testimonial') border-red-300 @enderror">{{ old('testimonial') }}</textarea>
                        @error('testimonial')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="partnership_since" name="partnership_since" type="text"
                            value="{{ old('partnership_since') }}" placeholder="Enter partnership since"
                    ...
                    <div>
                        <label for="key_achievements" class="field-label">
                            Key Achievements
                        </label>
                        <textarea id="key_achievements" name="key_achievements" rows="4" placeholder="Enter key achievements"
                    ...
                    <div>
                        <label for="photo" class="field-label req">
                            Photo
                        </label>
                        <div class="mt-2">

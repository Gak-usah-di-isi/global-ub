...
@section('title', 'Dashboard - Create Story')
...
@section('content')
    <div class="space-y-6">
        <nav class="text-[12px] font-medium flex items-center gap-1 text-slate-500">
            ...
            <span class="text-slate-700">Create Story</span>
        </nav>

        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <h1 class="text-[24px] md:text-[26px] font-semibold text-slate-800">Create Story</h1>
        ...
        <form id="create-form" action="{{ route('stories.store') }}" method="POST" enctype="multipart/form-data"
            class="space-y-8">
            @csrf
            <section class="bg-white shadow-card rounded-xl p-6 md:p-8">
                ...
                <div class="space-y-7">
                    ...
                    <div>
                        ...
                        <input id="category_story" name="category_story" type="text" value="{{ old('category_story') }}"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4" placeholder="Enter story description"
                            class="mt-2 w-full rounded-lg border border-[#E3E9F2] bg-[#F8FAFE] px-4 py-3 text-[13px] focus:border-primary-500 focus:bg-white focus:ring-primary-500/15 outline-none resize-vertical @error('description') border-red-300 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="thumbnail" class="field-label">Thumbnail</label>

...
@section('title', 'Dashboard - Create Carousel')
...
@section('content')
    <div class="space-y-6">
        <nav class="text-[12px] font-medium flex items-center gap-1 text-slate-500">
            ...
            <span class="text-slate-700">Create Carousel</span>
        </nav>
        ...
        @endif
        <form id="create-form" action="{{ route('carousels.store') }}" method="POST" enctype="multipart/form-data"
            class="space-y-8">
            @csrf
...
border-red-300
@enderror">{{ old('subtitle') }}</textarea>
                        @error('subtitle')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="help-text">

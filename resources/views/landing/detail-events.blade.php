            ...
            <div class="lg:w-7/12 gap-4 w-full justify-between flex h-auto flex-col">
                <div class="">
                    ...
                    <div class="flex flex-col gap-1 text-neutral-500 my-4">
                        <span class="flex items-center align-middle gap-2"><img src="{{ asset(config('app.icon_path') . '/calender.svg') }}"
                                alt="calender">{{ \Carbon\Carbon::parse($event->event_date)->format('F j, Y') }}</span>
                        <span class="flex items-center align-middle gap-2"><img src="{{ asset(config('app.icon_path') . '/clock.svg') }}"
                                alt="clock">{{ \Carbon\Carbon::parse($event->start_time)->format('g:i A') }} -
                            {{ \Carbon\Carbon::parse($event->end_time)->format('g:i A') }}</span>
                        <span class="flex items-center align-middle gap-2"><img src="{{ asset(config('app.icon_path') . '/location.svg') }}"
                                alt="location">{{ $event->location }}</span>
                        <span class="flex items-center align-middle text-neutral-500 gap-2"><img
                                src="{{ asset(config('app.icon_path') . '/user-black.svg') }}" alt="people">{{ $event->expected_attendees }}
                            Expected
                            Attendance</span>
                    </div>
                    {{-- <a
                ...
                <div class="flex items-end align-bottom gap-x-2">
                    <div
                        ...
                        <img src="{{ asset(config('app.icon_path') . '/fb.svg') }}" alt="facebook">
                    </div>
                    <div
                        class="flex items-center align-middle justify-center size-10 rounded-full text-neutral-600 bg-neutral-200 p-2">
                        <img src="{{ asset(config('app.icon_path') . '/ig.svg') }}" alt="instagram">
                    </div>
                    <div
                        class="flex items-center align-middle justify-center size-10 rounded-full text-neutral-600 bg-neutral-200 p-2">
                        <img src="{{ asset(config('app.icon_path') . '/tweet.svg') }}" alt="twitter">
                    </div>
                    <div
                        class="flex items-center align-middle justify-center size-10 rounded-full text-neutral-600 bg-neutral-200 p-2">
                        <img src="{{ asset(config('app.icon_path') . '/link.svg') }}" alt="url">
                    </div>
                </div>
            </div>
